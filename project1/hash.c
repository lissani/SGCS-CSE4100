...
// my func
// multipliciative hashing using golend ratio
unsigned
hash_int_2 (int i) 
{
  const unsigned A = 2654435769u;
  return (unsigned)(((unsigned long long)i * A) >> 32);
}
...
bool my_hash_compare(const struct hash_elem *a, const struct hash_elem *b, void *aux){
  struct hash_item *i_a= hash_entry(a, struct hash_item, elem);
  struct hash_item *i_b= hash_entry(b, struct hash_item, elem);
  return i_a->data < i_b->data;
}

void delete_hash_elem (struct hash_elem *e, void *aux){
  struct hash_item *i = hash_entry(e, struct hash_item, elem);
  free(i);
}
